using Module_5.HW_1.Models;
using Newtonsoft.Json;
using System.Net;
using System.Text;

namespace Module_5.HW_1
{
    public class Program
    {
        public static async Task Main(string[] args)
        {
            ...
            await CreateUser("Olha", "Developer");
            await UserPutRequest(1, "Olha", "Developer");
            await UserPatchRequest(2, "Vlad", "DevOps");
            await DeleteUser(2);
        ...
        public static async Task CreateUser(string name, string job)
        {
            using (var httpClient = new HttpClient { BaseAddress = new Uri("https://reqres.in/") })
            {
                ...
                if (result.StatusCode == HttpStatusCode.Created)
                {
                    ...
                    var createdUser = JsonConvert.DeserializeObject<CreateUser>(response);
        ...
        public static async Task UserPutRequest(int id, string? name, string? job)
        {
            using (var httpClient = new HttpClient { BaseAddress = new Uri("https://reqres.in/") })
            {
                ...
                if (result.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    var response = await result.Content.ReadAsStringAsync();
                    var updatedUser = JsonConvert.DeserializeObject<UpdateUser>(response);
        ...
        public static async Task RegisterUser(string? email, string? pass)
        {
            using (var httpClient = new HttpClient { BaseAddress = new Uri("https://reqres.in/") })
            {
                ...
                if (result.StatusCode == HttpStatusCode.OK)
                {
                    ...
                    var response = await result.Content.ReadAsStringAsync();
                    var token = JsonConvert.DeserializeObject<AuthorizationResponse>(response);
